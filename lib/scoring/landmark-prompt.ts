...
export function buildLandmarkDetectionPrompt(args: {
  ...
  return [
    ...
    ``,
    `BACKGROUND PARALLEL LINES (vanishing-point cross-check — optional)`,
    `If the image clearly shows background lines that you know are parallel in`,
    `the real world (fence rails, truck bed edges, barn boards, road markings),`,
    `report up to two such pairs in parallelLinePairs as`,
    `{ label, line1: [{x,y},{x,y}], line2: [{x,y},{x,y}] }. Each line is two`,
    `endpoints in image pixels. Omit when no clearly parallel pair is visible —`,
    `do NOT invent them. These are used only for perspective cross-check.`,
    ``,
