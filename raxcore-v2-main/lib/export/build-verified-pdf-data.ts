...
function buildMeasurementRow(
  ...
  // Derive provenance source from the highest-confidence measurement
  const primaryMethod = comparison.sources.length > 0 ? comparison.sources[0].method : null
  const source: 'ai' | 'heuristic' | 'human' | undefined =
    primaryMethod === 'quick_ai' ? 'ai' :
    primaryMethod === 'photo_polyline' || primaryMethod === 'three_d_point_cloud' || primaryMethod === 'manual_entry' ? 'human' :
    primaryMethod ? 'heuristic' :
    undefined

  const meshCircumValue = bestValueForMethod(fieldMeasurements, 'three_d_mesh_circumference' as AdvancedMeasurement['method'])
  ...
  return {
    ...
    warning,
    source,
    ...(meshCircumValue !== null ? { meshCircumferenceValue: meshCircumValue } : {}),
  }
}
