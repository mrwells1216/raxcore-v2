...
export interface CreateValidationResultParams {
  ...
  /**
   * Optional supervision context. When provided, `createValidationResult`
   * fires `onIntervalMiss` / `onHighConfidenceMiss` after the insert succeeds.
   * Each hook gates internally on meaningful deviation; passing these never
   * forces an event — it just enables the check.
   */
  predictionId?: string
...
export async function createValidationResult(
  ...
  // Phase 52 supervision hooks — fire if caller supplied prediction context.
  // Each hook gates internally on meaningful deviation; failures are logged, never thrown.
  if (params.predictionId) {
    const intervalAvailable =
      typeof params.predictedIntervalLow === 'number' &&
      typeof params.predictedIntervalHigh === 'number'

    if (intervalAvailable) {
      const outsideInterval =
        params.groundTruthGross < (params.predictedIntervalLow as number) ||
        params.groundTruthGross > (params.predictedIntervalHigh as number)
      if (outsideInterval) {
        try {
          await onIntervalMiss({
            predictionId: params.predictionId,
            buckId: params.buckId,
            predictedIntervalLow: params.predictedIntervalLow as number,
            predictedIntervalHigh: params.predictedIntervalHigh as number,
            actualScore: params.groundTruthGross,
            confidenceTier: params.confidenceTier ?? 'unknown',
            trustTier: params.trustTier,
            segment: params.segment,
            confidencePercent: params.confidencePercent ?? undefined,
          })
        } catch (hookError) {
          console.error('[Phase 52] onIntervalMiss hook failed:', hookError)
        }
      }
    }
    ...
    if (params.confidenceTier) {
      try {
        await onHighConfidenceMiss({
          ...
          trustTier: params.trustTier,
          missMagnitude: absErrorGross,
          intervalMiss: intervalAvailable
            ? params.groundTruthGross < (params.predictedIntervalLow as number) ||
              params.groundTruthGross > (params.predictedIntervalHigh as number)
            : false,
          predicted: params.predictedGross,
          actual: params.groundTruthGross,
          segment: params.segment,
        })
      } catch (hookError) {
        console.error('[Phase 52] onHighConfidenceMiss hook failed:', hookError)
      }
    }
  }
