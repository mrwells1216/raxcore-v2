...
export function PhotogrammetryPanel() {
  ...
  // Auto-wire assets into the store when Luma reconstruction completes
  const prevStatusRef = useRef(reconstructionStatus)
  useEffect(() => {
    if (prevStatusRef.current === reconstructionStatus) return
    prevStatusRef.current = reconstructionStatus
    if (reconstructionStatus !== 'completed') return

    const meshAsset = reconstructionAssets.find(a => a.type === 'mesh_glb')
    const pcAsset = reconstructionAssets.find(a =>
      a.type === 'point_cloud_xyz' || a.type === 'point_cloud_ply' || a.type === 'point_cloud_csv'
    )
    if (meshAsset?.url) setGlbUrl(meshAsset.url)
    if (pcAsset?.url) {
      fetch(pcAsset.url)
        .then(r => r.text())
        .then(text => loadPointCloudText(text, pcAsset.url.split('/').pop() ?? 'point-cloud'))
        .catch(err => console.warn('[reconstruct] auto-load point cloud failed', err))
    }
  }, [reconstructionStatus, reconstructionAssets, setGlbUrl, loadPointCloudText])
