...
export interface MeasureStore {
  ...
  setHoveredZoneId: (id: FieldId | null) => void
  ...
  /** Record a circumference measured via mesh-plane cross-section. Only stores if closed ring and valid length. */
  setMeshCircumferenceMeasurement: (fieldId: FieldId, inchLength: number, ringClosed: boolean, confidence: number) => void
...
export const useMeasureStore = create<MeasureStore>()((set, get) => ({
  ...
  setMeshCircumferenceMeasurement: (fieldId, inchLength, ringClosed, confidence) => set((s) => {
    if (!isFiniteNumber(inchLength) || inchLength <= 0 || !ringClosed) return {}
    const m = s.measurements3D[fieldId]
    const tier = confidence >= 0.7 ? 'high' : 'medium'
    const warnings = m.warnings.filter(w => w !== MESH_FALLBACK_WARNING)
    return {
      measurements3D: {
        ...s.measurements3D,
        [fieldId]: {
          ...m,
          method: 'three_d_mesh_circumference' as MeasurementMethod,
          inchLength,
          confidence: tier,
          finalized: true,
          warnings,
