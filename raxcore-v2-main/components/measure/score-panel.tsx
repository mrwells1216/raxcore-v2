...
async function resizeThumbnail(src: string, maxPx: number, quality: number): Promise<string> {
...
function MethodBadge({ method }: { method: string }) {
  const label =
    ...
    method === 'three_d_mesh_circumference'    ? 'Mesh Circ' :
    method === 'quick_ai'                      ? 'AI' :
  ...
  const color =
    ...
    method === 'three_d_mesh_circumference'    ? '#d94a4a' :
...
export function ScorePanel() {
  ...
  const hoveredZoneId = useMeasureStore(s => s.hoveredZoneId)
  const setHoveredZoneId = useMeasureStore(s => s.setHoveredZoneId)
  ...
  const handleExportVerifiedPDF = async () => {
    ...
    try {
      ...
      const exportData = buildVerifiedPdfExportData({
        ...
        photoThumbnails: await (async () => {
          const rawSources = [
            ...(photoDataUrl ? [photoDataUrl] : []),
            ...reconstructionAssets
              .filter(a => a.type === 'preview_image' && a.url)
              .map(a => a.url!),
          ].slice(0, 4)
          const thumbs: string[] = []
          for (const src of rawSources) {
            try { thumbs.push(await resizeThumbnail(src, 256, 0.70)) } catch { /* skip */ }
          }
          return thumbs
        })(),
      })
            ...
            >
              <div
                ...
                style={{
                  ...
                  outline: hoveredZoneId === row.id ? `1px solid ${row.color}` : undefined,
                  borderRadius: hoveredZoneId === row.id ? '4px' : undefined,
                }}
              >
