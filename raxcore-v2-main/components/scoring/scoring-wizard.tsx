...
import { detectionToScanFeedback, type ScanFeedback } from '@/lib/detection/detection-to-scan-feedback'
import { validateSubject, canProceedToScoring, mapGroupToSection, mapScanAngleToSection } from '@/lib/capture/subject-validation'
...
export function ScoringWizard({ initialMode, userId, onComplete }: ScoringWizardProps) {
  ...
  const [preScoreChecking, setPreScoreChecking] = useState(false)
  const [subjectBlocked, setSubjectBlocked] = useState(false)
  const preScoreDebounceRef = useRef<ReturnType<typeof setTimeout> | null>(null)
  ...
  const runPreScoreDetection = useCallback(async (imgs: GridImage[]) => {
    if (imgs.length === 0) {
      setScanFeedback(null)
      setSubjectBlocked(false)
      return
    }
    setPreScoreChecking(true)
    try {
      const imageUrls = imgs.slice(0, 4).map(img => img.url)
      ...
      const feedback = detectionToScanFeedback(result)
      setScanFeedback(feedback)

      const declaredSections = imgs.map(img =>
        img.group ? mapGroupToSection(img.group as any) : mapScanAngleToSection(img.angleType as any)
      )
      const validation = validateSubject({
        imageCount: imgs.length,
        declaredSections,
        aiDetectedDeer: result.accepted,
        aiDetectedAntlers: result.accepted,
        isSmartScanMode: inputMode === 'smart-scan',
      })
      setSubjectBlocked(!canProceedToScoring(validation))
    } catch {
      // Never block submit on detection errors
      setScanFeedback(null)
      setSubjectBlocked(false)
    } finally {
      setPreScoreChecking(false)
    }
  }, [inputMode])

  useEffect(() => {
    if (preScoreDebounceRef.current) clearTimeout(preScoreDebounceRef.current)
    if (gridImages.length === 0) {
      setScanFeedback(null)
      setSubjectBlocked(false)
      return
    }
    preScoreDebounceRef.current = setTimeout(() => {
      runPreScoreDetection(gridImages)
    }, 1200)
    return () => {
      if (preScoreDebounceRef.current) clearTimeout(preScoreDebounceRef.current)
    }
  }, [gridImages, runPreScoreDetection])
  ...
  const canSubmit = gridImages.length >= 1 && (intakeQuality?.canProceed ?? true) && !subjectBlocked
  ...
  return (
    <div className="max-w-2xl mx-auto pb-32 space-y-3">
      ...
      {/* ── 5b. Pre-Score Review ─────────────────────────────────────────── */}
      {(preScoreChecking || scanFeedback) && (
        <Section label="Photo Validation">
          <p className="text-[10px] text-muted-foreground mb-3">
            AI rack validation runs against your uploaded photos, not the camera.
          </p>
          {preScoreChecking && !scanFeedback && (
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              <Loader2 className="h-3.5 w-3.5 animate-spin shrink-0" />
              Checking photos for scorable rack...
            </div>
          )}
          {scanFeedback && (
            <div className="space-y-2">
              <div className="flex items-start gap-2.5">
                <span
                  ...
                  style={{
                    background: scanFeedback.color === 'green' ? '#4fc36e'
                      : scanFeedback.color === 'yellow' ? '#e0a030'
                      : '#d94a4a',
                  }}
                />
                <div className="space-y-0.5">
                  <p className="text-xs font-semibold text-foreground">{scanFeedback.headline}</p>
                  <p className="text-[11px] text-muted-foreground">{scanFeedback.subline}</p>
                </div>
              </div>
              {!scanFeedback.accepted && (
                <div className="flex items-start gap-2.5 rounded-xl border border-red-500/25 bg-red-500/8 px-3 py-2.5">
                  ...
                  <div className="flex-1 space-y-2">
                    <p className="text-xs text-red-400">{scanFeedback.nextPrompt}</p>
                    <button
                      type="button"
                      onClick={() => {
                        handleGridChange([])
                      ...
                      className="text-[11px] font-semibold text-red-400 underline underline-offset-2"
                    >
                      Retake / Re-upload
                    </button>
                  </div>
              ...
              {scanFeedback.accepted && scanFeedback.nextPrompt && (
                <p className="text-[11px] text-muted-foreground">
                  <span className="font-medium text-foreground/70">Photo coverage suggestions:</span>{' '}
                  {scanFeedback.nextPrompt}
                </p>
              )}
            </div>
          )}
